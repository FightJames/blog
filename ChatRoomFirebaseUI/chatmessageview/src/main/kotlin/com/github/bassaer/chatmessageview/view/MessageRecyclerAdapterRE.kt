...
class MessageRecyclerAdapterRE(context: Context, resource: Int, private var objects: List<Any>, private var attribute: Attribute) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder?, position: Int) {
        if (holder is DateViewHolder) {
            var dateViewHolder = holder
            dateViewHolder.dateLabelText?.text = objects[position] as String
            ...
            var messageViewHolder = holder as MessageViewHolder
            val message: Message = objects[position] as Message
            if (position > 0) {
                objects[position - 1].let {
                    if (it is Message && it.user.getId() == message.user.getId()) {
                        //If send same person, hide username and icon.
//                        if (position == 1) {
//                            Log.d("message user same", it.user.getName().toString())
//                        }
...
//                if (position == 1) {
//                    Log.d("message user same", "icon hide false")
//                }
            ...
            //Set Padding
            messageViewHolder.messageRoot?.setPadding(0, messageTopMargin, 0, messageBottomMargin)
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, position: Int): RecyclerView.ViewHolder {
        //Log.d("pass", position.toString())
        var view: View? = null
        if (position == TEXTTYPE) {
            lateinit var dateViewHolder: DateViewHolder
            view = layoutInflater.inflate(R.layout.date_cell, parent, false)
            dateViewHolder = DateViewHolder(view)
            ...
            var v = layoutInflater.inflate(R.layout.message_layout, parent, false)
            messageViewHolder = MessageViewHolder(v)
...
//bindHolder
    ...
    internal inner class MessageViewHolder : RecyclerView.ViewHolder {
        ...
        var messageRoot: LinearLayout? = null

        constructor(itemView: View?) : super(itemView) {
            messageRoot = itemView!!.root
        }

    }

    internal inner class DateViewHolder : RecyclerView.ViewHolder {
